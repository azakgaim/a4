...
const EVENTS: IEvent[] = [
  {
    ...
    sessions: [
      ...
      },
      {
        ...
        voters: ['johnpapa', 'bradgreen', 'igorminar', 'martinfowler', 'anthony']
      },
      {
        id: 3,
...
export class EventsService {
  ...
  createEvent(event: IEvent) {
    let nextId = Math.max.apply(null, e => EVENTS.map(i => i.id));
    nextId = nextId + 1;
  ...
  /*
  resolve router hanlder
    - prefetch data for a component
    - conduct some checks prior to loading a component
  */
  ...
  searchSessions(searchTerm: string): any {
    ...
    EVENTS.forEach(event => {
      // get a list of sessions where a name of a session contains a term specified by
      // a user for an event being checked
      let matchingSessions = event.sessions.filter(session => {
        return session.name.toLocaleLowerCase().indexOf(term) > -1;
      });

      // adding an event id to a ression
      matchingSessions = matchingSessions.map((session: any) => {
        session.eventId = event.id;
        return session;
      });

      // add found sessions of an event being checked to a result
      result = result.concat(matchingSessions);
    });

    // a list of sessions is found. now let's raise an event.
